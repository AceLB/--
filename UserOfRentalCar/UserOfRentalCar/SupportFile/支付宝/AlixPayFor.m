...
-(void)goodAalixPay:(GoodInfo *)goods
{
	...
	order.seller = seller;
	order.tradeNO = goods.tradeNO; //订单ID（由商家自行制定）
	order.productName = goods.productName; //商品标题
	order.productDescription = goods.productDescription; //商品描述
	order.amount = [NSString stringWithFormat:@"%.2f",goods.amount]; //商品价格
	order.notifyURL =  @"http://www.xxx.com"; //回调URL
    
//	[order.extraParams setValue:@"fw" forKey:@"type"];
	//应用注册scheme,在AlixPayDemo-Info.plist定义URL types,用于安全支付成功后重新唤起商户应用
	NSString *appScheme = @"UserOfRentalCar";
